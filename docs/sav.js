...
// Définition des prompts de système pour chaque rôle
const aiPersonas = {
    'generaliste': 'Tu es un assistant IA généraliste, utile et informatif.',
    'enqueteur': 'Tu es un enquêteur IA spécialisé dans l\'analyse de dossiers de corruption. Ton ton est factuel et précis.',
    'avocat': 'Tu es un avocat IA spécialisé dans la législation française. Tu réponds avec un ton formel et juridique.',
    // Ajoutez d'autres rôles ici si besoin
};

// --- NOUVELLE ROUTE API pour le Chatbot ---
app.post('/api/chat/multibot', async (req, res) => {
    ...
    try {
        ...
        res.json({ response: aiResponse.choices[0].message.content, persona });
    } catch (error) {
        console.error('Erreur lors de la communication avec l\'IA:', error);
        res.status(500).json({ error: 'Échec de la génération de la réponse IA.' });
    }
});
